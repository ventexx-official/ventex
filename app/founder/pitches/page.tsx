...
export default function MyPitchesPage() {
 ...
 <button onClick={() => setConfirmDelete(null)} className="absolute top-4 right-4 p-1.5 hover:bg-[var(--bg)] rounded-lg text-[var(--text2)]"><X className="w-4 h-4" /></button>
 ...
 <p className="text-sm text-[var(--text2)] text-center mb-1">You're about to permanently delete:</p>
 <p className="text-sm font-bold text-[var(--text)] text-center mb-6 px-4 truncate">"{confirmDelete.title}"</p>
 ...
 <button onClick={() => setConfirmDelete(null)} className="flex-1 py-3 border-[0.5px] border-[var(--border)] rounded-2xl font-bold text-[var(--text)] hover:bg-[var(--bg)] text-sm">Cancel</button>
 <button onClick={handleDelete} className="flex-1 py-3 bg-red-500 text-[var(--text)] rounded-2xl font-bold hover:bg-red-600 active:scale-95 transition-all text-sm flex items-center justify-center gap-2">
 ...
 <div>
 <h1 className="text-2xl md:text-3xl font-black text-[var(--text)] tracking-tighter uppercase">My Pitches</h1>
 <p className="text-[var(--text2)] text-sm mt-1">{pitches.length} pitch{pitches.length !== 1 ? 'es' : ''} total</p>
 </div>
 <Link
 href="/founder/create-pitch"
 className="bg-[var(--text)] text-[var(--bg)] px-5 py-3 rounded-2xl font-bold flex items-center gap-2 hover:bg-black active:scale-95 transition-all w-full sm:w-auto justify-center"
 >
 ...
 className="w-full pl-10 pr-4 py-2.5 bg-[var(--card-bg)] border-[0.5px] border-[var(--border)] rounded-xl text-sm focus:outline-none focus:ring-1 focus:ring-[#222222] text-[var(--text)]"
 />
 </div>
 <select
 value={statusFilter}
 onChange={e => setStatusFilter(e.target.value)}
 className="bg-[var(--card-bg)] border-[0.5px] border-[var(--border)] rounded-xl px-4 py-2.5 text-sm text-[var(--text)] focus:outline-none focus:ring-1 focus:ring-[#222222]"
 >
 ...
 {filtered.length === 0 ? (
 <div className="bg-[var(--card-bg)] rounded-3xl border-[0.5px] border-[var(--border)] p-16 text-center">
 <FileText className="w-10 h-10 text-[var(--text3)] mx-auto mb-4" />
 <h3 className="font-bold text-[var(--text)] mb-2">{search || statusFilter !== 'all' ? 'No pitches match your filters' : "You haven't created any pitches yet"}</h3>
 <p className="text-sm text-[var(--text2)] mb-6">{search ? 'Try a different search term.' : 'Start building your investor-ready pitch profile.'}</p>
 {!search && statusFilter === 'all' && (
 <Link href="/founder/create-pitch" className="inline-flex items-center gap-2 bg-[var(--text)] text-[var(--bg)] px-6 py-3 rounded-2xl font-bold text-sm hover:bg-black transition-colors">
 ...
 <div
 key={pitch.id}
 className={`bg-[var(--card-bg)] p-5 rounded-3xl border-[0.5px] border-[var(--border)] shadow-sm hover:shadow-md transition-all flex items-center justify-between group ${deletingId === pitch.id ? 'opacity-40 pointer-events-none' : ''}`}
 >
 ...
 <h3 className="font-bold text-[var(--text)] text-sm">{pitch.title || 'Untitled Pitch'}</h3>
 <StatusBadge status={pitch.status} />
 </div>
 <div className="flex items-center gap-4 text-[11px] font-medium text-[var(--text2)] flex-wrap">
 ...
 {pitch.industry && <span className="hidden md:inline bg-[var(--bg)] px-2 py-0.5 rounded text-[10px]">{pitch.industry}</span>}
 {pitch.company_stage && <span className="hidden md:inline bg-[var(--bg)] px-2 py-0.5 rounded text-[10px]">{pitch.company_stage}</span>}
 </div>
 </div>
 </div>

 <div className="flex items-center gap-2 ml-4 flex-shrink-0">
 <Link href={`/founder/create-pitch?id=${pitch.id}`} className="hidden sm:block px-3 py-1.5 bg-[var(--bg)] text-[var(--text)] text-xs font-bold rounded-xl hover:bg-[#e5e5e5] transition-colors">Edit</Link>
 <Link href={`/pitch/${pitch.id}`} className="hidden sm:block px-3 py-1.5 bg-[var(--text)] text-[var(--bg)] text-xs font-bold rounded-xl hover:bg-black transition-colors">View</Link>
 ...
 {openMenuId === pitch.id && (
 <div className="absolute right-0 top-full mt-1 w-48 bg-[var(--card-bg)] rounded-2xl shadow-xl border-[0.5px] border-[var(--border)] z-50 py-1">
 <Link href={`/founder/create-pitch?id=${pitch.id}`} onClick={() => setOpenMenuId(null)} className="flex items-center gap-3 px-4 py-2.5 text-sm font-medium text-[var(--text)] hover:bg-[var(--bg)]"><Edit2 className="w-4 h-4 text-[var(--text2)]" />Edit pitch</Link>
 <Link href={`/pitch/${pitch.id}`} onClick={() => setOpenMenuId(null)} className="flex items-center gap-3 px-4 py-2.5 text-sm font-medium text-[var(--text)] hover:bg-[var(--bg)]"><ExternalLink className="w-4 h-4 text-[var(--text2)]" />View public page</Link>
 <button onClick={() => handleDuplicate(pitch)} className="flex items-center gap-3 w-full px-4 py-2.5 text-sm font-medium text-[var(--text)] hover:bg-[var(--bg)]"><Copy className="w-4 h-4 text-[var(--text2)]" />Duplicate</button>
