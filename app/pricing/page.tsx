...
export default function PricingPage() {
 ...
 <div>
 <p className="mono mb-3 text-xs font-bold uppercase tracking-[.16em] text-[#666666] ">Pricing</p>
 ...
 <p className="mt-4 max-w-2xl text-sm leading-6 text-[#666666] ">Ventex Premium is marketplace access only. Investor Accounts are separate and built for pitch discovery, deal flow, and founder contact.</p>
 </div>
 ...
 <button key={value} onClick={() => setMode(value as PriceMode)} className={`rounded-full px-4 py-2 text-xs font-black ${mode === value ? 'bg-[var(--text)] text-[var(--bg)] dark:bg-[var(--card-bg)] dark:text-[var(--text)]' : 'text-[#666666] '}`}>
 {label}
 </button>
 ))}
 </div>
 ...
 {openFaq === index ? <p className="pb-4 text-sm leading-6 text-[#666666] ">{answer}</p> : null}
 </div>
 ))}
 </div>
 </section>
