    ...
    return (
      <main className="grid min-h-screen place-items-center bg-[#0A0A0C] px-4 text-center text-white">
        <div>
          ...
          <p className="mx-auto mt-4 max-w-xl text-sm leading-6 text-neutral-400">Monthly pitch nights are built but disabled until the owner enables Ventex Live from Admin Ã¢â€ â€™ Feature Flags.</p>
        </div>
      </main>
    );
  }
      ...
      <div className="mx-auto max-w-6xl space-y-10">
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <form onSubmit={submitFounder} className="rounded-2xl bg-[var(--card-bg)] p-6 space-y-3">
            ...
            <input placeholder="Funding ask Ã¢â€šÂ¹" value={founderForm.askInr} onChange={(e) => setFounderForm((p) => ({ ...p, askInr: e.target.value }))} className="w-full rounded-xl border p-3" />
          ...
          <form onSubmit={submitJudge} className="rounded-2xl bg-[var(--card-bg)] p-6 space-y-3">
            ...
            <select value={judgeForm.ticket} onChange={(e) => setJudgeForm((p) => ({ ...p, ticket: e.target.value }))} className="w-full rounded-xl border p-3">{["<Ã¢â€šÂ¹10L", "Ã¢â€šÂ¹10-50L", "Ã¢â€šÂ¹50L-1Cr", ">Ã¢â€šÂ¹1Cr", "Flexible"].map((s) => <option key={s}>{s}</option>)}</select>
