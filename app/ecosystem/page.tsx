...
export default function EcosystemPage() {
 ...
 <p className="mt-2 text-sm font-medium text-[var(--text2)]">Startup density by state, powered by live Ventexx pitch data.</p>
 </header>

 <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_320px]">
 <section className="rounded-3xl border border-[var(--border)] bg-[var(--card-bg)] p-4">
 ...
 <div className="rounded-3xl border border-[var(--border)] bg-[var(--card-bg)] p-6">
 <div className="mb-3 text-xs font-black uppercase tracking-widest text-[var(--text2)]">Top states</div>
 <div className="space-y-3">
 {rows.slice(0, 8).map(([state, item]) => (
 <div key={state} className="flex items-center justify-between gap-4 text-sm">
 <span className="font-bold text-[var(--text)]">{state}</span>
