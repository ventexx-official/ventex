    ...
    return (
      <div className="min-h-screen bg-[var(--bg)] px-4 py-10">
        <div className="max-w-3xl mx-auto bg-[var(--card-bg)] border-[0.5px] border-[var(--border)] rounded-3xl p-6 md:p-8">
          ...
          <button
            onClick={() => router.push('/')}
            className="mt-6 bg-[var(--text)] text-[var(--text)] px-5 py-2.5 rounded-full text-sm font-bold hover:bg-black transition-colors"
          >
            Go Home
          </button>
        </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-6">
                ...
                <div className="mt-5 rounded-2xl border border-[var(--border)] bg-[#F8F8F8] p-4">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {['Pitch Submitted', 'First Investor Interest', 'Featured in Battle', 'Marketplace Seller', 'Ventex Live Participant'].map((badge) => (
                      <span key={badge} className={`rounded-full px-2 py-1 text-[10px] font-black ${badges.includes(badge) ? 'bg-[var(--text)] text-[var(--text)]' : 'bg-[var(--card-bg)] text-[var(--text2)]'}`}>
                        {badge}
                      </span>
                    ))}
                  </div>
            ...
            {isOwnProfile && (
              <div className="flex flex-col sm:flex-row gap-2 sm:items-center">
                <Link
                  href="/settings"
                  className="inline-flex items-center justify-center gap-2 bg-[var(--text)] text-[var(--text)] px-5 py-2.5 rounded-full text-sm font-bold hover:bg-black transition-colors"
                >
